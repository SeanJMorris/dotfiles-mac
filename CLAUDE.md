...
2. **Firefox switcher surfaces the wrong window — a fixable *race*, NOT a hard limitation like Excel (diagnosed 2026-07-12).** Symptom: with `core-window-switcher.js`, all Firefox windows appear in the switcher, but selecting any window other than the most-recently-used (MRU) one often surfaces the MRU window instead of the one picked. It happens less if you pause before releasing the switcher modifier, and only affects non-MRU windows. **Why this is different from the Excel case (limitation #1):** Firefox windows are *standard* macOS windows, so BTT genuinely *can* activate a specific one — proven by testing BTT's discrete **"Activate Specific Window"** action, which raised any non-MRU Firefox window 100% reliably. So the raise mechanism is sound; the failure is isolated to the switcher's **trigger-on-release** path (`BTTWindowSwitcherTriggerOnRelease: true`) — activating at the instant the modifier is released races and lands on the MRU window. **Workaround:** pause briefly on the highlighted window before releasing. **Real fix:** report to the BTT dev (Andreas) with the repro — *"Activate Specific Window works 100%, but the release-triggered switcher surfaces the MRU window unless I pause first"* — it's a switcher bug, not a macOS/app constraint. **Diagnostic tips for next time:** (a) to get a window's exact title as BTT matches it, run in Script Editor: `tell application "System Events" to tell process "firefox" to get name of every window` — the switcher's label prepends the app name and differs from the real accessibility title; (b) parentheses/other regex-special characters in a title break BTT's title match, so test with a plain-text title or use "contains".
3. BTT window switcher can't show up on every monitor. Gemini and GPT said that it was not possible for the BTT Window switcher to show up on every monitor.
